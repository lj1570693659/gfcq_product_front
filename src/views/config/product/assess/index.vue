<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="tableData" style="width: 100%; margin-bottom: 20px  margin-bottom: 20px" row-key="value" stripe size="medium" :tree-props="{ children: 'children' }">
      <el-table-column prop="label" label="评价标准" />
      <el-table-column prop="evaluateDimensions" type="hidden" label="评价标准" />
    ...
    >
      ...
      >
        ...
        <el-form-item label="评价标准：" prop="label">
          <el-input
            v-model="temp.label"
            size="small"
            autocomplete="off"
    ...
    fetchData() {
      ...
      getLists().then(response => {
        ...
        response.forEach((k, v) => {
          var item = { tagId: k.id, label: k.evaluateDimensions, evaluateDimensions: k.evaluateDimensions, evaluateCriteria: k.evaluateCriteria, weight: k.weight, scoreCriteria: k.scoreCriteria, parent: k.evaluateId, remark: k.remark, value: k.id, children: [] }
          var children = []
          var childKey = []
          if (k.childLevel.length > 0) {
            k.childLevel.forEach((kc, vc) => {
              childKey[k.id] = 0
              childKey[kc.id] = vc + 1
              var itemc = { tagId: kc.id, label: kc.evaluateCriteria, evaluateDimensions: k.evaluateDimensions, scoreCriteria: kc.scoreCriteria, parent: kc.evaluateId, weight: kc.weight.toFixed(2), remark: kc.remark, value: String(k.id) + '-' + String(kc.id), children: [], childKey: childKey }
              children[kc.id] = itemc
            })
            item.children = children
          }
          Vue.set(this.tableData, k.id, item)
          Vue.set(this.locationData, v, { id: k.id, name: k.evaluateDimensions })
        },
        )
      })
    },
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          var data = {
            id: 0,
            evaluateCriteria: this.temp.label,
            weight: this.temp.weight,
            evaluateId: this.temp.location,
            scoreCriteria: this.temp.scoreCriteria,
            remark: this.temp.remark
          }
          if (this.temp.location === '0') {
            data.evaluateDimensions = this.temp.label
          } else {
            data.evaluateDimensions = this.tableData[this.temp.location].evaluateDimensions
          }
