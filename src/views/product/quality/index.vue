...
import { getQualifyLists, modifyQualifyData } from '@/api/product/task'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getQualifyLists(this.listQuery).then(response => {
    ...
    confirmEdit(row) {
      row.edit = false
      modifyQualifyData({ id: row.id, taskStatus: row.taskStatus, remark: row.remark }).then(response => {
        this.$message({
          message: '任务信息变更',
          type: 'success'
        })
